...
- **`GET /` & `GET /api/health`** — Health check endpoint returning status and timestamp.
...
2. **Configure Environment Variables**:
   ...
   - `SUPABASE_SERVICE_ROLE_KEY`: Your Supabase Service Role Key
   - `OPENROUTER_API_KEY`: Your OpenRouter API Key
   - `ALLOWED_ORIGINS`: Allowed origins e.g., `*` or `https://pulse-frontend.vercel.app,http://localhost:5173`
